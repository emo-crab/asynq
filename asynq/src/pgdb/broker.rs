...
use super::entity::tasks::TaskState;
use crate::base::keys::TaskState as BaseTaskState;
use crate::base::Broker;
use crate::error::{Error, Result};
use crate::pgdb::entity::{servers, stats, tasks, workers, Queues, Servers, Stats, Tasks, Workers};
...
impl Broker for PostgresBroker {
  ...
  async fn dequeue(&self, queues: &[String]) -> Result<Option<TaskMessage>> {
    ...
    for queue in queues {
      ...
      if let Some(t) = task {
        ...
        let msg = self.decode_task_message(&t.encoded_task)?;
        return Ok(Some(msg));
      }
    }
  ...
  async fn mark_as_complete(&self, msg: &TaskMessage) -> Result<()> {
    ...
    if let Some(t) = task {
      ...
      active_model.encoded_task = Set(encoded);
      active_model.updated_at = Set(now.into());
      active_model.update(self.db()).await?;
    }
  ...
  async fn read_aggregation_set(
    ...
    for t in tasks {
      if let Ok(msg) = self.decode_task_message(&t.encoded_task) {
        messages.push(msg);
      }
    }
    Ok(messages)
  ...
  async fn write_server_state(&self, server_info: &ServerInfo, ttl: Duration) -> Result<()> {
    ...
    if let Some(s) = existing {
      ...
      active_model.server_info = Set(server_info_bytes);
      active_model.update(self.db()).await?;
    } else {
      let new_server = servers::ActiveModel {
        ...
        status: Set(servers::ServerStatus::Active),
        expires_at: Set(expires_at.into()),
        server_info: Set(server_info_bytes),
        tenant_id: Set(self.tenant_id()),
      };
