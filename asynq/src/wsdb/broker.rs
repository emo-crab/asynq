...
use crate::wsdb::message::{
  ArchiveRequest, ClientMessage, DequeueRequest, EnqueueUniqueRequest, RetryRequest,
  ScheduleRequest, ScheduleUniqueRequest, ServerMessage,
};
use crate::wsdb::{ws_broker::CLOSE_FRAME_TIMEOUT_MS, WebSocketBroker};
...
impl Broker for WebSocketBroker {
  ...
  async fn add_to_group(&self, _task: &Task, _group: &str) -> Result<TaskInfo> {
    Err(Error::not_supported(
      "add_to_group not yet supported via WebSocket",
    ))
  }

  /// Add unique to group - Not yet implemented
  async fn add_to_group_unique(
    ...
    Err(Error::not_supported(
      "add_to_group_unique not yet supported via WebSocket",
    ))
  }

  /// List groups - Not yet implemented
  async fn list_groups(&self, _queue: &str) -> Result<Vec<String>> {
    Err(Error::not_supported(
      "list_groups not yet supported via WebSocket",
    ))
  }

  /// Aggregation check - Not yet implemented
  async fn aggregation_check(
    ...
    Err(Error::not_supported(
      "aggregation_check not yet supported via WebSocket",
    ))
  }

  /// Read aggregation set - Not yet implemented
  async fn read_aggregation_set(
    ...
    Err(Error::not_supported(
      "read_aggregation_set not yet supported via WebSocket",
    ))
  }

  /// Delete aggregation set - Not yet implemented
  async fn delete_aggregation_set(&self, _queue: &str, _group: &str, _set_id: &str) -> Result<()> {
    Err(Error::not_supported(
      "delete_aggregation_set not yet supported via WebSocket",
    ))
  }
